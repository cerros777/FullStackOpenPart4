...
test.only('new blog added', async () => {
...
test.only('adding a blogs fails when token is not provided', async () => {
...
test('blog without title returns 400 Bad Request', async () => {
  ...
  try {
    const response = await api
      ...
      .send(newBlog)
      .timeout({
        response: 20000,  // Increase response timeout
        deadline: 30000, // Increase deadline
      });
...
test('a blog can be updated', async () => {
  ...
  // Get the blogs after update
  const blogsAtEnd = await api.get('/api/blogs').timeout({ response: 20000, deadline: 30000 });

  // Verify the blog is updated
